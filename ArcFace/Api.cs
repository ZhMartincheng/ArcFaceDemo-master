	...
	public static class Api
	{
		...
		public static void FaceMatch(Bitmap bitmap, int featureDataIndex = -1)
		{
			...
			try
			{
				...
				IntPtr pDetectResult = Marshal.AllocHGlobal(Marshal.SizeOf(faceModel));
				...
				for (int i = 0; i < detectResult.nFace; i++)
				{
					...
					if ((ErrorCode)ArcWrapper.ExtractFeature(_REngine, ref imageData, ref faceFeatureInput, out  faceModel) != ErrorCode.Ok)
						continue;
					...
					foreach (var item in _FaceLib.Items.OrderByDescending(ii => ii.OrderId))
					{
						...
						float score = 0f;
						ArcWrapper.Match(_REngine, ref fm, ref faceModel, out  score);
						if (score > 0.7)
						{
							matched = true;
		...
		public static void AddFace(string id, byte[] featureData)
		{
			...
			{
				MessageBox.Show(ex.Message, "错误", MessageBoxButtons.YesNo, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1);
				
			}

			MessageBox.Show("注册成功", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
		}   
	}
}